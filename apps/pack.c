...
static bool pack_config_load(const String path, PackConfig* out) {
  ...
  data_read_json(g_dataReg, data, g_allocHeap, g_packConfigMeta, readFlags, outMem, &result);
  if (result.error) {
...
static PackGatherResult pack_gather_update(
  ...
  dynarray_for_t(&pack->assets, PackAsset, packAsset) {
    ...
    // NOTE: Add more supported asset roots here.
    (void)assetMan;
...
AppType app_ecs_configure(CliApp* app) {
  cli_app_register_desc(app, string_lit("Playground asset packer"));
