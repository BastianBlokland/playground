...
bind_instance_data(0) readonly uniform InstanceSkinned {
  StandardSkinnedData[c_objMaxInstances] u_instances;
};

bind_internal(0) out f32v2 out_texcoord;
bind_internal(1) out flat f32v4 out_data;
...
void main() {
  ...
  const f32m4x3 instanceSkinMat =
      standard_skin_mat(u_instances[in_instanceIndex], vert.jointIndices, vert.jointWeights);
