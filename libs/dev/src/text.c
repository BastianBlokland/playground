...
ecs_view_define(RendererCreateView) {
  ecs_access_with(RendCameraComp);
...
static GeoMatrix dev_text_view_proj(const RendCameraComp* cam, const UiVector res) {
  const f32 aspect = res.width / res.height;
  return rend_camera_view_proj(cam, aspect);
}
...
ecs_system_define(DevTextRenderSys) {
  ...
  for (ecs_view_itr_reset(rendererItr); ecs_view_walk(rendererItr);) {
    ...
    const RendCameraComp*      camera   = ecs_view_read_t(rendererItr, RendCameraComp);
    ...
    const GeoMatrix viewProj = dev_text_view_proj(camera, res);
