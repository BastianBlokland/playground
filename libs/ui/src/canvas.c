...
ecs_comp_define(UiOwnerComp) { EcsEntityId owner; };
...
static void ui_renderer_create(EcsWorld* world, const EcsEntityId window) {
  ...
  ecs_world_add_t(world, rendObjEntity, UiOwnerComp, .owner = window);

  const RendObjectFlags objFlags = RendObjectFlags_NoInstanceFiltering;
...
ecs_view_define(OwnerView) { ecs_access_read(UiOwnerComp); }

ecs_system_define(UiLifetimeSys) {
  EcsView* ownerView = ecs_world_view_t(world, OwnerView);
  for (EcsIterator* itr = ecs_view_itr(ownerView); ecs_view_walk(itr);) {
    const UiOwnerComp* ownerComp = ecs_view_read_t(itr, UiOwnerComp);
...
ecs_module_init(ui_canvas_module) {
  ...
  ecs_register_system(UiLifetimeSys, ecs_view_id(OwnerView));
...
ui_canvas_create(EcsWorld* world, const EcsEntityId window, const UiCanvasCreateFlags flags) {
  ...
  ecs_world_add_t(world, canvasEntity, UiOwnerComp, .owner = window);
  return canvasEntity;
}
