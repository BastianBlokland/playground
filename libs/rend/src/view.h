...
typedef struct sRendView {
  EcsEntityId   camera;
  GeoVector     origin;
  RendTagFilter filter;
  GeoPlane      frustum[4]; // Left, Right, Top, Bottom.
} RendView;

RendView
rend_view_create(EcsEntityId camera, GeoVector origin, const GeoMatrix* viewProj, RendTagFilter);
...
bool rend_view_visible(
    const RendView*, RendTags objTags, const GeoBox* objAabb, const RendSettingsComp*);
