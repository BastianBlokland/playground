 ...
 */
ecs_system_define(AssetSoundUnloadSys) {
...
ecs_module_init(asset_sound_module) {
  ...
  ecs_register_system(AssetSoundUnloadSys, ecs_view_id(UnloadView));
}
...
void asset_load_sound_bin(
  ...
  data_read_bin(
      g_dataReg, src->data, g_allocHeap, g_assetSoundMeta, readFlags, mem_var(sound), &result);
