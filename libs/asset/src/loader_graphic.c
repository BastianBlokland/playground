 ...
 */
ecs_system_define(AssetGraphicInitSys) {
  ...
  Error:
    // NOTE: 'AssetGraphicComp' will be cleaned up by 'AssetGraphicUnloadSys'.
    ecs_world_remove_t(world, entity, AssetGraphicInitComp);
  }
}
 ...
 */
ecs_system_define(AssetGraphicUnloadSys) {
...
ecs_module_init(asset_graphic_module) {
  ...
  ecs_register_system(AssetGraphicInitSys, ecs_view_id(ManagerView), ecs_view_id(InitView));
  ecs_register_system(AssetGraphicUnloadSys, ecs_view_id(UnloadView));
}
