 ...
 */
ecs_system_define(AssetScriptUnloadSys) {
...
ecs_module_init(asset_script_module) {
  ...
  ecs_register_system(AssetScriptUnloadSys, ecs_view_id(ScriptUnloadView));
}
...
void asset_data_init_script(void) {
  ...
  data_reg_field_t(g_dataReg, AssetScriptComp, strings, data_prim_t(StringHash), .container = DataContainer_HeapArray, .flags = DataFlags_StringRequired);
  // clang-format on
...
void asset_load_script_bin(
  ...
  data_read_bin(
      g_dataReg, src->data, g_allocHeap, g_assetScriptMeta, readFlags, mem_var(script), &result);
