 ...
 */
ecs_system_define(AssetLocaleUnloadSys) {
...
ecs_module_init(asset_locale_module) {
  ...
  ecs_register_system(AssetLocaleUnloadSys, ecs_register_view(LocaleUnloadView));
}
...
void asset_load_locale(
  ...
  if (result.error) {
    ...
    goto Ret;
    // NOTE: 'AssetLocaleComp' will be cleaned up by 'AssetLocaleUnloadSys'.
  }
