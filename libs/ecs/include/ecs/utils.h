...
/**
 * Get a read-only pointer to a component on an entity or null.
 *
 * Pre-condition: view has 'Read' access to the given component type.
 */
#define ecs_utils_maybe_read_t(_WORLD_, _VIEW_NAME_, _ENTITY_, _TYPE_)                             \
  ((const _TYPE_*)ecs_utils_maybe_read(                                                            \
      ecs_world_view_t((_WORLD_), _VIEW_NAME_), (_ENTITY_), ecs_comp_id(_TYPE_)))

const void* ecs_utils_maybe_read(EcsView*, EcsEntityId, EcsCompId);
