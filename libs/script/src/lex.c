...
static ScriptLexKeyword g_lexKeywords[] = {
    {.id = string_static("if"), .idHash = ScriptHash_if, .token = ScriptTokenKind_If},
    {.id = string_static("else"), .idHash = ScriptHash_else, .token = ScriptTokenKind_Else},
    {.id = string_static("var"), .idHash = ScriptHash_var, .token = ScriptTokenKind_Var},
    {.id = string_static("while"), .idHash = ScriptHash_while, .token = ScriptTokenKind_While},
    {.id = string_static("continue"), .idHash = ScriptHash_continue, .token = ScriptTokenKind_Continue},
    {.id = string_static("break"), .idHash = ScriptHash_break, .token = ScriptTokenKind_Break},
    {.id = string_static("for"), .idHash = ScriptHash_for, .token = ScriptTokenKind_For},
    {.id = string_static("return"), .idHash = ScriptHash_return, .token = ScriptTokenKind_Return},
    {.id = string_static("to"), .idHash = ScriptHash_to, .token = ScriptTokenKind_To},
};
// clang-format on
...
static String script_lex_number_positive(String str, ScriptToken* out) {
  ...
  while (!string_is_empty(str)) {
    ...
    case 't':
      if (str.size >= 2 && *string_at(str, 1) == 'o') {
        goto NumberEnd; // Support 1to3.
      }
      // Fallthrough.
    default:
...
NumberEnd:
  ...
  if (UNLIKELY(result < i32_min || result > i32_max)) {
    return *out = script_token_diag(ScriptDiag_NumberOutOfRange), str;
  }
  if (UNLIKELY(lastChar == '_')) {
    return *out = script_token_diag(ScriptDiag_NumberEndsWithSeparator), str;
  }
  out->kind       = ScriptTokenKind_Number;
  out->val_number = (i32)result;
  return str;
}
...
String script_lex(String str, StringTable* stringtable, ScriptToken* out, const ScriptLexFlags fl) {
  while (!string_is_empty(str)) {
    ...
    case '&':
      ...
      if (script_peek(str, 1) == '=') {
        return out->kind = ScriptTokenKind_AmpEq, script_consume_chars(str, 2);
      }
    ...
    case '|':
      ...
      if (script_peek(str, 1) == '=') {
        return out->kind = ScriptTokenKind_PipeEq, script_consume_chars(str, 2);
      }
      return *out = script_token_diag(ScriptDiag_InvalidChar), script_consume_chars(str, 1);
    case '?':
      if (script_peek(str, 1) == '?') {
    ...
    case '\t':
      ...
      continue;
    case 't':
      if (script_peek(str, 1) == 'o') {
        return out->kind = ScriptTokenKind_To, script_consume_chars(str, 2);
      }
      // Fallthrough.
    default:
...
String script_token_str_scratch(const ScriptToken* token) {
  ...
  case ScriptTokenKind_Semicolon:
    return string_lit(";");
  case ScriptTokenKind_AmpEq:
    return string_lit("&=");
  case ScriptTokenKind_AmpAmp:
    return string_lit("&&");
  case ScriptTokenKind_PipeEq:
    return string_lit("|");
  ...
  case ScriptTokenKind_To:
    return string_lit("to");
