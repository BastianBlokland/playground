...
static ScriptCompileError compile_intr(Context* ctx, const Target tgt, const ScriptExpr e) {
  ...
  case ScriptIntrinsic_RangeMin:
    return compile_intr_unary(ctx, tgt, ScriptOp_RangeMin, args);
  case ScriptIntrinsic_RangeMax:
    return compile_intr_unary(ctx, tgt, ScriptOp_RangeMax, args);
  case ScriptIntrinsic_RangeFromTo:
    return compile_intr_binary(ctx, tgt, ScriptOp_RangeFromTo, args);
  case ScriptIntrinsic_Clamp:
    return compile_intr_binary(ctx, tgt, ScriptOp_Clamp, args);
  case ScriptIntrinsic_Contains:
    return compile_intr_binary(ctx, tgt, ScriptOp_Contains, args);
