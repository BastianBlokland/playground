...
static void script_builtin_init(void) {
  ...
  {
    const String       name   = string_lit("range_min");
    const String       doc    = string_lit("Retrieve the min component of a range.");
    const ScriptMask   ret    = script_mask_num;
    const ScriptSigArg args[] = {
        {string_lit("v"), script_mask_num_range},
    };
    script_builtin_func_add(name, ScriptIntrinsic_RangeMin, doc, ret, args, array_elems(args));
  }
  {
    const String       name   = string_lit("range_max");
    const String       doc    = string_lit("Retrieve the max component of a range.");
    const ScriptMask   ret    = script_mask_num;
    const ScriptSigArg args[] = {
        {string_lit("v"), script_mask_num_range},
    };
    script_builtin_func_add(name, ScriptIntrinsic_RangeMax, doc, ret, args, array_elems(args));
  }
  {
  ...
  }
  {
    ...
    const String       doc    = string_lit("Clamp a value to a range.");
    const ScriptMask   ret    = script_mask_any;
    const ScriptSigArg args[] = {
        {string_lit("v"), script_mask_num},
        {string_lit("r"), script_mask_num_range},
    };
    script_builtin_func_add(name, ScriptIntrinsic_Clamp, doc, ret, args, array_elems(args));
  }
  {
    const String       name   = string_lit("contains");
    const String       doc    = string_lit("Test if a value is contained within a range.");
    const ScriptMask   ret    = script_mask_any;
    const ScriptSigArg args[] = {
        {string_lit("v"), script_mask_num | script_mask_num_range},
        {string_lit("r"), script_mask_num_range},
    };
    script_builtin_func_add(name, ScriptIntrinsic_Contains, doc, ret, args, array_elems(args));
  }
  {
...
static ScriptIntrinsic token_op_binary(const ScriptTokenKind kind) {
  ...
  case ScriptTokenKind_To:
    return ScriptIntrinsic_RangeFromTo;
  default:
...
typedef struct {
  ...
  ScriptSym           builtinConstSyms[script_builtin_consts_max];
...
ScriptExpr script_read(
  ...
  mem_set(array_mem(ctx.builtinConstSyms), 0xFF);
