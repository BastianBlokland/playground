...
#define VM_VISIT_OP_SIMPLE                                                       \
  ...
  VM_OP_SIMPLE_UNARY(       RangeMin,           script_val_num_range_min        )\
  VM_OP_SIMPLE_UNARY(       RangeMax,           script_val_num_range_max        )\
  VM_OP_SIMPLE_BINARY(      RangeFromTo,        script_val_num_range_from_to    )\
  VM_OP_SIMPLE_BINARY(      Clamp,              script_val_clamp                )\
  VM_OP_SIMPLE_BINARY(      Contains,           script_val_contains             )\
...
Dispatch:
  ...
  case ScriptOp_Extern: {
    ...
    panicHandlerIp = ip; // Store the ip for diagnostic info in case a panic happens.
