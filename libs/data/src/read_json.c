...
#define result_success() (DataReadResult){0}
...
static void data_read_json_union(const ReadCtx* ctx, DataReadResult* res) {
  ...
  *data_union_tag(&decl->val_union, ctx->data) = choice->tag;
  ...
  if (!emptyChoice) {
    ...
    if (choiceDecl->kind == DataKind_Struct && choice->meta.container == DataContainer_None &&
        !data_struct_inline_field(&choiceDecl->val_struct)) {
      /**
