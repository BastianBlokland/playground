...
spec(utils_hash) {
  ...
  it("can compute the hash of a nested union") {
    typedef enum {
      HashUnionTag_Value,
      HashUnionTag_Next,
    } HashUnionTag;

    typedef struct sHashUnionNested {
      HashUnionTag tag;
      union {
        i32                      data_value;
        struct sHashUnionNested* data_next;
      };
    } HashUnionNested;

    data_reg_union_t(reg, HashUnionNested, tag);
    data_reg_choice_t(reg, HashUnionNested, HashUnionTag_Value, data_value, data_prim_t(i32));
    data_reg_choice_t(
        reg,
        HashUnionNested,
        HashUnionTag_Next,
    ...
    const u32 hash = data_hash(reg, data_meta_t(t_HashUnionNested), DataHashFlags_None);
    check(hash != 0);
  }
